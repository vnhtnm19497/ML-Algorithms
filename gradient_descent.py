...
def check_grad(x):
	...
	for i in range (len(x)):
		...
		x1[i] += eps
		x2[i] -= eps
		g[i] = (cost(x1) - cost(x2))/(2*eps) #Approximal value 

	g_grad = grad(x) # Formula's value
...
def gradient_descent(x_init, learning_rate, iteration):
	...
	for i in range(iteration):
		...
		if np.linalg.norm(grad(x_new))/len(x_new)< 0.3: #when to stop GD algorithm 
			break
	return x_list
...
#Draw animation for visualizing GD algorithm
...
plt.legend(('Value in each GD iteration', 'Solution by formula', 'Inital value for GD'), loc=(0.52,0.01))
